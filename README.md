...
Prefer deserialization libraries that require explicit field declarations, such as `Thoth.Json` (with `Thoth.Json`, use explicit `Decode.object/Decode.field`-style decoders, not the "auto" ones using reflection under the hood) instead, so that structural mismatches between expected and actual data are caught eagerly rather than silently swallowed.
